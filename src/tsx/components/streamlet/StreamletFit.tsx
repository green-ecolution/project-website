  ...
  return (
    <section className="px-4 max-w-208 mx-auto my-20 md:px-6 lg:my-28 lg:max-w-screen-lg xl:my-36 xl:max-w-screen-xl">
        ...
        {fitCriteria.map((criterion) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-grey-900/80 leading-relaxed">
                {t(`fit.criteria.${criterion.id}`)}
              </span>
            </li>
          )
        })}
      ...
      <div className="mt-8 rounded-2xl bg-green-light-100/70 border border-green-light-900/20 p-6 lg:p-8">
        <h3 className="font-lato font-semibold text-grey-900 mb-2">{t('fit.cargo.title')}</h3>
        <p className="text-grey-900/70 leading-relaxed">{t('fit.cargo.description')}</p>
      </div>
    </section>
  )
}
