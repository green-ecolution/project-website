...
Drilling and burying went smoothly. Each site took about 10 to 15 minutes: the TBZ drills the hole with an earth auger, we place the sensor, connect it and fill the hole back in. Afterwards nothing of the sensor is visible above ground. Still, there was one lesson for the next installations: a flashlight now belongs on the packing list.
...
The sensors send their readings over LoRaWAN, a radio network that gets by on very little energy and achieves long ranges. That keeps maintenance low and makes it easy to add further sites. One of the open questions before the installation, however, was whether the connection would also hold up reliably from within the soil.
