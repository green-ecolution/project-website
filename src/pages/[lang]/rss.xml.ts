...
import { articleSlug, getArticles } from '../../lib/blog'
...
export async function GET(context: APIContext) {
  ...
  return rss({
    ...
    trailingSlash: false,
    // The language belongs in the feed: a reader showing both feeds of both
    // languages would otherwise have no way to tell them apart.
    customData: `<language>${language}</language>`,
    items: articles.map((entry) => ({
      title: entry.data.title,
      description: entry.data.summary,
      pubDate: entry.data.date,
      link: `/${language}/blog/${articleSlug(entry)}`,
    })),
  })
}
